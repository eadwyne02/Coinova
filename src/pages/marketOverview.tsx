    ...
    <div>
      <div className="fixed top-0 right-0 left-0 lg:left-64 bg-[#23282e] px-4 pt-4 pb-2">
        ...
        <div className="relative mb-3">
          ...
          <input type="text" placeholder="Search Cryptocurrencies" value={query} onChange={e => setQuery(e.target.value)} className="w-full pl-12 pr-4 py-2.5 rounded-xl bg-[#151923] text-white placeholder-white/60 outline-none focus:ring-1 focus:ring-white/20 text-sm"/>
          {query && (
          ...
          <p className="text-center text-white/40 mt-10 text-sm">No coins found for "{query}"</p>
            ...
            return (
              <div className="flex items-center justify-between pt-3.5 pb-5 border-b border-white/[0.06] last:border-none" key={coin.id}>
                <div className="flex items-center gap-2.5 min-w-[110px]">
                  <img src={coin.image} alt={coin.name} className="w-9 h-9 md:w-11 md:h-11 rounded-full object-cover"/>
                  <div>
                    <p className="text-sm md:text-[16px] font-semibold text-white leading-tight">{coin.symbol.toUpperCase()}</p>
                    <p className="text-[11px] md:text-[14px] text-white/45 mt-0.5">{coin.name}</p>
                  </div>
                </div>
                <div className="flex-1 flex justify-center">
                ...
                <div className="text-right min-w-[80px]">
                  <p className="text-sm md:text-[16px] font-semibold text-white leading-tight">
                    ${coin.current_price.toLocaleString()}
                  </p>
                  <p className={`text-[11px] md:text-[13px] font-medium mt-0.5 ${isPositive ? 'text-green-400' : 'text-red-400'}`}>
