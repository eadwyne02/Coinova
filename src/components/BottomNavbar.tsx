...
export default function BottomNav() {
  ...
  const navItems = [
    {
      ...
      label: 'Home',
      active: (
        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="#00a9f9" stroke="none">
          <path d="M12 2.1L2 9.5V21a1 1 0 0 0 1 1h6v-7h6v7h6a1 1 0 0 0 1-1V9.5L12 2.1z"/>
        </svg>
      ),
      inactive: (
        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.35)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div className="bg-[#1a1f27] border-t border-white/[0.06] px-2 py-1 pb-safe">
      <div className="flex justify-around items-center">
        {navItems.map(({ path, label, active, inactive }) => {
          ...
          return (
            ...
            >
              {active_ && (
                <span className="absolute inset-0 rounded-xl bg-[#00a9f9]/[0.08]" />
              )}
              {active_ ? active : inactive}
              <span className={`text-[10px] font-medium transition-colors ${active_ ? 'text-[#00a9f9]' : 'text-white/30'}`}>
                {label}
              </span>
            </button>
          )
